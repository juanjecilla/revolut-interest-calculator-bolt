...
function CalculatorPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-10">
      ...
      <div className="mb-8">
        <ComparisonChart amount={amount} dark={false} />
      </div>
...
const navLinkStyle = ({ isActive }: { isActive: boolean }) => ({
  ...
  color: isActive ? 'var(--bg-surface)' : 'var(--text-2)',
  fontWeight: 600,
  fontSize: 13,
  textDecoration: 'none',
  display: 'inline-flex',
    ...
    >
      ...
      >
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center gap-4">
          <Calculator className="w-5 h-5 flex-shrink-0" style={{ color: 'var(--brand-ink)' }} />
          <span
            className="font-bold text-base tracking-tight"
            style={{ color: 'var(--text-1)', marginRight: 8 }}
          >
            Revolut
          </span>

          <nav aria-label="Primary" style={{ display: 'flex', gap: 4, flex: 1 }}>
            <NavLink to="/" end style={navLinkStyle}>
              <Calculator className="w-3.5 h-3.5" />
              Calculator
            </NavLink>
            <NavLink to="/compound" style={navLinkStyle}>
